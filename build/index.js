...
class NReplMcpServer {
    ...
    setupRequestHandlers() {
        ...
        `;
                ...
                const namespacesArray = JSON.parse(namespaces
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            try {
                switch (request.params.name) {
                    case 'connect': {
                        ...
                        if (!args || typeof args.host !== 'string' || typeof args.port !== 'number') {
                            throw new McpError(ErrorCode.InvalidParams, 'host and port parameters are required');
                        }
                    ...
                    case 'get_ns_vars': {
                        ...
                        if (!args || typeof args.ns !== 'string') {
                            throw new McpError(ErrorCode.InvalidParams, 'ns parameter must be a string');
                        }
                        const result = await this.nreplClient.eval(`(into {} (for [[sym v] (ns-publics '${args.ns})] [sym {:meta (meta v) :value (deref v)}]))`);
                        return {
                            content: [{ type: 'text', text: result }],
                        };
                    }
                ...
                throw new McpError(ErrorCode.InternalError, `nREPL error: ${error instanceof Error ? error.message : String(error)}`);
            }
        });
    }
    async ensureNReplClient() {
        if (!this.host || !this.port) {
            throw new McpError(ErrorCode.InternalError, 'Not connected to nREPL server - use connect tool first');
        }
        if (!this.nreplClient) {
            this.nreplClient = new NReplClient(this.port);
            await this.nreplClient.clone(); // Create initial session
